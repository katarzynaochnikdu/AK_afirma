"use strict";
// Wersja STATYCZNA (GitHub Pages) — dane z window.DEMO_DATA, filtrowanie w przegladarce.
...
const DATA = window.DEMO_DATA || { months: [], products: [], cells: [] };

function reportFor(from, to, product) {
  const lo = Number(from.replace("-", ""));
  const hi = Number(to.replace("-", ""));
  return DATA.cells.filter((c) => {
    const ym = c.year * 100 + c.month;
    if (ym < lo || ym > hi) return false;
    if (product && c.productCode !== product) return false;
    return true;
  });
}
...
function init() {
  $("status").textContent =
    `Pozycji (komórek) w zestawie demo: ${fmtInt(DATA.cells.length)} · dane fikcyjne`;
...
function render() {
  const from = $("fromMonth").value || DATA.months[0];
  const to = $("toMonth").value || DATA.months[DATA.months.length - 1];
  const product = $("product").value;
  if (!from || !to) return;

  const cells = reportFor(from, to, product);
...
function buildTable(cells, cols) {
  ...
  if (cells.length === 0) {
    thead.innerHTML = ""; tbody.innerHTML = ""; table.hidden = true; empty.hidden = false;
    return;
  }
